...
void sdl2_input::terminate( bool on ) {
    is_terminated = on;
}

bool sdl2_input::terminated() {
    return is_terminated;
}

void sdl2_input::restart( bool on ) {
    is_restarted = on;
}

bool sdl2_input::restarted() {
    return is_restarted;
}

void sdl2_input::debug( bool on ) {
    is_debugged = on;
}

bool sdl2_input::debugged() {
    return is_debugged;
}

void sdl2_input::process( bool has_focus ) {
    SDL_GetMouseState(&x, &y);
    left = SDL_GetMouseState(0, 0);
        ...
        switch (ev.type) {
            ...
            break; case SDL_KEYUP:
                ...
                switch (ev.key.keysym.scancode) {
                    ...
                    break; case SDL_SCANCODE_UP:
                        //game->apply_input(Game::InputForce_SHOOT, ev.key.state == SDL_PRESSED ? 1 : 0);
                    break; case SDL_SCANCODE_ESCAPE:
                        //game->apply_input(Game::InputForce_START, ev.key.state == SDL_PRESSED ? 1 : 0);
                        terminate(1);
                    break; case SDL_SCANCODE_F5:
                        restart(1);
                    break; case SDL_SCANCODE_F1:
                        debug(1);
                }
            ...
            break; case SDL_WINDOWEVENT:
                switch (ev.window.event) {
                    ...
                    break; case SDL_WINDOWEVENT_RESTORED:
                        SDL_Log("Window %d restored", ev.window.windowID);
                    break; case SDL_WINDOWEVENT_ENTER:
                    ...
                    break; case SDL_WINDOWEVENT_CLOSE:
...
int sdl2_input::mouse_x() {
    return x;
}

int sdl2_input::mouse_y() {
    return y;
}

int sdl2_input::mouse_left() {
    return left;
}

int sdl2_input::keyboard( int x ) {
    //assert( x >= 0 && x < SDL_NUM_SCANCODES);
    //assert( keystates );
    return keystates[ x ]; // _UP, _DOWN, _LEFT, _RIGHT
}
...
130, 0x082, SDL_SCANCODE_LOCKINGCAPSLOCK
131, 0x083, SDL_SCANCODE_LOCKINGNUMLOCK
132, 0x084, SDL_SCANCODE_LOCKINGSCROLLLOCK
