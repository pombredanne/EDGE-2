...
static void playfile(const char *fname)
{
   ...
   if( SDL_CreateWindowAndRenderer(video->width, video->height, SDL_WINDOW_RESIZABLE, &window, &renderer) ) {
   ...
   else  // software surface
   {
        ...
        overlay = SDL_CreateTexture(renderer, SDL_PIXELFORMAT_IYUV, SDL_TEXTUREACCESS_TARGET, video->width, video->height);
    ...
    while (!quit && THEORAPLAY_isDecoding(decoder))
    {
        ...
        if (video && (video->playms <= now))
        {
            ...
            else
            {
                ...
                SDL_UpdateYUVTexture(overlay, &dstrect, y, w, u, w / 2, v, w / 2 );
