...
void config_eval() {
	// [ref] http://es.cppreference.com/w/cpp/language/operator_precedence

	opers.insert({"or", oper_t {false, 1, false}});
	opers.insert({"and", oper_t {false, 2, false}});

	opers.insert({"|", oper_t {false, 3, false}});
	opers.insert({"^", oper_t {false, 4, false}});
	opers.insert({"&", oper_t {false, 5, false}});

	opers.insert({"==", oper_t{false, 6, false}});
	opers.insert({"!=", oper_t{false, 6, false}});

	opers.insert({"<",  oper_t{false, 7, false}});
	opers.insert({">",  oper_t{false, 7, false}});
	opers.insert({"=<", oper_t{false, 7, false}});
	opers.insert({"=>", oper_t{false, 7, false}});

	opers.insert({"+", oper_t{false,  9, false}});
	opers.insert({"-", oper_t{false,  9, false}});
	...
	opers.insert({"+", oper_t{false,  11, true}});
	opers.insert({"-", oper_t{false,  11, true}});

	opers.insert({"!", oper_t{false, 12, true}});

	opers.insert({"!!", oper_t{true, 13, true}}); // factorial
	...
	funcs.insert({"or", func_args(2, [](args_t v)
	{
		return bool(v[0]) || bool(v[1]);
	})});
	funcs.insert({"and", func_args(2, [](args_t v)
	{
		return bool(v[0]) && bool(v[1]);
	})});
	...
	funcs.insert({"pow", func_args(2, [](args_t v)
	{
		return pow(v[0], v[1]);
	})});
