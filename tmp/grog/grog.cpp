...
/*
struct idi : public std::string {
    idi() : std::string("0")
    {}
    idi( int i ) : std::string( std::to_string(i) )
    {}
    idi( const std::string &i ) : std::string(i)
    {}
};
*/
...
MEDEA_DEFINE( line &it, (it.id, it.text, it.choices) );
...
MEDEA_DEFINE( choice &it, (it.id, it.text) );
...
MEDEA_DEFINE( chat &it, (it.lines, it.choices) );
...
MEDEA_DEFINE( fact &it, (it.id, it.text) );
...
MEDEA_DEFINE( actor &it, (it.id, it.lines, it.facts, it.stages) );
...
int main() {
    ...
    std::cout << medea::to_json(grog::get_actor("gnu")) << std::endl;
