...
#include "spatial.hpp" // vec3

#include "gl/gl.hpp"
#include "gl/nocopy.hpp"
#include "gl/disable.hpp"
#include "texture/texture.hpp" // texture, texturemap
...
namespace eve
{
    namespace style
    {
        ...
        struct texture : nocopy
        {
            ...
            explicit
            texture( const char *const pathFile, const bool high_quality = true ) : id( 0 )
            {
#if 0
                ...
                {
                    eve::texture &tx = ( map[ pathFile ] = map[ pathFile ] );
            ...
            explicit
            texture( const eve::texture &t ) : id(t.id)
            {
                glEnable(GL_TEXTURE_2D);
            ...
            {
                GLuint id = (GLuint)_id;
                glBindTexture(GL_TEXTURE_2D, id );
                //std::cout << "texture #" << id << std::endl;
            }

            static void apply( const std::string &pathFile )
            {
                if( !pathFile.size() )
                    return;
                ...
                static std::map< std::string, eve::texture > map;
        ...
        struct color : nocopy
        {
             //explicit color( const eve::vec4 &c ) { glColor4f( c.x, c.y, c.z, c.a ); }
             explicit color( const eve::vec3 &c, float a ) { glColor4f( c.x, c.y, c.z, a ); }
             explicit color( const eve::vec3 &c ) { glColor4f( c.x, c.y, c.z, 1.f ); }
        ...
        using namespace eve_color_scheme;
