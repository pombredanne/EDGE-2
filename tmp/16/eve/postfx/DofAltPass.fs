...
uniform float focalDepth = 1.f;  //focal distance value in meters, but you may use autofocus option below
uniform float focalLength = 500.f; //focal length in mm
uniform float fstop = 3.f; //f-stop value
...
void main() 
{
    ...
    float depth = linearize(TEXTURE2D(bgl_DepthTexture,GL_TEXCOORD.xy).x);
    
    if (depthblur)
    {
        depth = linearize(bdepth(GL_TEXCOORD.xy));
    }
    ...
    vec2 noise = rand(GL_TEXCOORD.xy)*namount*blur;
    ...
    {
        col = TEXTURE2D(bgl_RenderedTexture, GL_TEXCOORD.xy).rgb;
    }
    ...
    else
    {
        ...
        {   
            ...
            for (int j = 0 ; j < ringsamples ; j += 1)   
            {
                ...
                col += color(GL_TEXCOORD.xy + vec2(pw*w,ph*h),blur)*mix(1.0,(float(i))/(float(rings)),bias)*p;  
