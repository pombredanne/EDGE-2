...
struct vec2 {
    ...
    friend float dotp(const vec2 &a, const vec2 &b) { return a.x * b.x + a.y * b.y; }
...
struct vec3 {
    ...
    friend float dotp(const vec3 &a, const vec3 &b) { return a.x * b.x + a.y * b.y + a.z * b.z; }
...
struct vec4 {
    ...
    friend float dotp(const vec4 &a, const vec4 &b) { return a.x * b.x + a.y * b.y + a.z * b.z + a.w * a.w; }
    ...
    template<typename ostream>
    inline friend ostream &operator<<(ostream &out, const vec4 &v) {
...
struct mat4 {
    ...
    template<typename ostream>
    inline friend ostream &operator <<(ostream &out, const mat4 &t) {
        return out << "mat4("
...
struct Texture {
    unsigned int id;
    int target, width, height, depth, internalFormat, format, type;

    Texture() : id(), target(), width(), height(), depth(), internalFormat(), format(), type() {}
...
void begin() const { bind(0); }
void end() const { unbind(0); }
...
struct FBO {
    ...
    bool autoDepth = 1;
    bool resizeViewport = 1;
    int newViewport[4], oldViewport[4];
    int renderbufferWidth = 0, renderbufferHeight = 0;
    std::vector<unsigned int> drawBuffers;

    FBO(bool autoDepth = true, bool resizeViewport = true) : autoDepth(autoDepth), resizeViewport(resizeViewport)
    {
        newViewport[0] = newViewport[1] = newViewport[2] = newViewport[3] = 0;
        oldViewport[0] = oldViewport[1] = oldViewport[2] = oldViewport[3] = 0;
    }
...
#define glsl0(...) "#version 000\n" #__VA_ARGS__
#define glsl3(...) "#version 330\n" #__VA_ARGS__
#define glsl4(...) "#version 400\n" #__VA_ARGS__
...
struct Shader {
    ...
    std::string sources, errors;

    std::map< int, std::string > cache;
    void reload() { *this = Shader(); for(auto &it : cache) { *this = shader( it.first, it.second.c_str() ); } link(); }

    Shader() : id() {}
    ~Shader();

    Shader &shader(int type, const std::string &source);
    Shader &vertexShader(const std::string &source) { return shader(GL_VERTEX_SHADER, source); }
    Shader &fragmentShader(const std::string &source) { return shader(GL_FRAGMENT_SHADER, source); }
    Shader &geometryShader(const std::string &source) { return shader(GL_GEOMETRY_SHADER, source); }
    Shader &tessControlShader(const std::string &source) { return shader(GL_TESS_CONTROL_SHADER, source); }
    Shader &tessEvalShader(const std::string &source) { return shader(GL_TESS_EVALUATION_SHADER, source); }

    std::string link(); // return errors if any
    void use() const { glUseProgram(id); }
    void unuse() const { glUseProgram(0); }

void begin() const { use(); }
void end() const { unuse(); }
    ...
    void uniformv(const char *name, const float *ptr, int count = 1) { glUniform1fv(uniform(name), count, ptr); }
};
...
struct Buffer {
    ...
    GLsizei size() const { return (GLsizei)data.size(); }
...
struct VAO {
    ...
    struct BufferHolderImpl : BufferHolder {
        ...
        GLsizei size() const { return (GLsizei)buffer.size(); }
    };
    ...
    const Shader *shader;
    std::shared_ptr<BufferHolder> vertices;
    std::shared_ptr<BufferHolder> indices;
    ...
    VAO &create(const Shader &shader, const Buffer<Vertex> &vbo) {
        ...
        this->shader = &shader;
        vertices.reset( new BufferHolderImpl<Vertex>(vbo) );
        indices.reset();
    ...
    VAO &create(const Shader &shader, const Buffer<Vertex> &vbo, const Buffer<Index> &ibo) {
        ...
        this->shader = &shader;
        vertices.reset( new BufferHolderImpl<Vertex>(vbo) );
        indices.reset( new BufferHolderImpl<Index>(ibo) );
        stride = sizeof(Vertex);
    ...
    template <typename T>
    VAO &attribute(int location, int count, bool normalized = GL_FALSE) {
        bind();
        glEnableVertexAttribArray(location);
        glVertexAttribPointer(location, count, TypeToOpenGL<T>::value, normalized, stride, (char *)0 + offset);
        unbind();
    ...
    template <typename T>
    VAO &attribute(const char *name, int count, bool normalized = GL_FALSE) {
        return attribute<T>( shader->attribute(name), count, normalized );
    }
