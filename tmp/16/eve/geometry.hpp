...
//glTexParameteri(GL_TEXTURE_2D, GL_TEXTURE_WRAP_T, GL_CLAMP);
    ...
    namespace geometry
    {
        ...
        {
            explicit
            line( const eve::vec3 &v = eve::float110() )
            {
                glBegin(GL_LINES);
                    glVertex3f(0,0,0);
        ...
        struct points : nocopy
        {
            ...
            explicit points( const std::vector< eve::vec3 > &pointList, int size = 16 )
            {
        ...
        struct lines : nocopy
        {
            ...
            explicit lines( const std::vector< eve::vec3 > &pointList )
            {
                size_t N = pointList.size();
        ...
        struct curve : nocopy // spliner
        {
            ...
            explicit
            curve( const eve::spline<eve::vec3> &knotList, const float &detail01 = 0.5f )
            {
                assert( knotList.size() >= 4 );

                const int STEPS = int( knotList.size() * int(detail01 * 7.0f + 1.f) ); //0..1 -> 1..8 -> size..size*8
                ...
                glBegin( GL_LINE_STRIP );
                    ...
                    for (int i = 0; i <= STEPS; ++i)
                    {
                        ...
                        eve::vec3 point = knotList.atdt( dt01 );
        ...
        struct arrow : nocopy
        {
            explicit arrow( const eve::vec2 &v = eve::vec2(1,1) )
            {
        ...
        struct coord3
        {
            coord3( const eve::vec3 &pt )
            {
                style::dotted line;
                disable::blending noshading;

                std::vector<eve::vec3> v( 2, pt );
        ...
        */
...
#if 0
                // glut
                eve::strings strs = eve::string( str ).tokenize("\r\n");
        ...
        {
            explicit vector( const eve::vec3 &direction )
            {
                glBegin( GL_LINES );
            ...
            camera()
            {
                if( 0 )
                {
                    // rlyeh's art :o)
                    {
                        ...
                        matrix::scale scy( eve::vec3(1,2,1) );
                        cone c( 0.f );
                    }
                    glTranslatef( 1.5f, 0.f, 0.f );
                    {
                        matrix::scale scx( eve::vec3(2,1,1) );
                        cube box;
                    }
                ...
                else
                {
                    ...
                    eve::matrix::scale sc( eve::vec3(1,2,0.75) );
                    eve::matrix::rotate_x rx( -180.f );
                    eve::matrix::scale sc2( eve::vec3(1,-1,1) );
                    ...
                    for(int currentReel = 0; currentReel < 2; currentReel++)
                    {
                        float radius    = 0.25f;
                        int resolution  = 32;
                        eve::vec3 reelCenter = eve::vec3(0, -0.25f + (currentReel== 0 ? 0.0f : 0.5f), -0.5f);
                        glBegin(GL_LINES);
                            eve::vec3 s = reelCenter - eve::vec3(0,0.25f,0);
                            ...
                            for(int i = 0; i < resolution; i++)
                            {
                                ...
                                eve::vec3 p = eve::vec3(0, ex*radius, -why*radius*3) + reelCenter;
                                glVertex3f(p.x, p.y, p.z);  //end of this line...
                                glVertex3f(p.x, p.y, p.z);  //and start of the next
                            }
                            eve::vec3 f = reelCenter + eve::vec3(0,0.25f,0);
                    ...
                    float lensOff = 0.3f;
                    float lensOut = 0.2f;
                    eve::vec3 v0    =   eve::vec3( 0.5f,  0.5f, -0.5f);
                    eve::vec3 v1    =   eve::vec3(-0.5f,  0.5f, -0.5f);
                    eve::vec3 v2    =   eve::vec3(-0.5f,  0.5f,  0.5f);
                    eve::vec3 v3    =   eve::vec3( 0.5f,  0.5f,  0.5f);

                    eve::vec3 l0    =   v0 + eve::vec3( lensOut,0,0) + eve::vec3(0,lensOut,0) + eve::vec3(0,0,-lensOff);
                    eve::vec3 l1    =   v1 + eve::vec3(-lensOut,0,0) + eve::vec3(0,lensOut,0) + eve::vec3(0,0,-lensOff);
                    eve::vec3 l2    =   v2 + eve::vec3(-lensOut,0,0) + eve::vec3(0,lensOut,0) + eve::vec3(0,0,lensOff);
                    eve::vec3 l3    =   v3 + eve::vec3( lensOut,0,0) + eve::vec3(0,lensOut,0) + eve::vec3(0,0,lensOff);
        ...
        struct manipulator : nocopy
        {
            manipulator()
            {
                eve::matrix::scale s1(1.0);  // to do: matrix::fixed size.. no matter camera distance
                eve::disable::lighting no_lights;
                eve::style::thick thicker;

                //float detail01 = 1.f;
                {
                    eve::style::blue b;
                    eve::geometry::circle( detail01 );
                    {
                    eve::style::green g;
                    eve::matrix::rotate_x x( 90 );
                    eve::geometry::circle( detail01 );
                    }
                    {
                    eve::style::red r;
                    eve::matrix::rotate_y y( 90 );
                    eve::geometry::circle( detail01 );
                    }
                }
                eve::matrix::scale s2(1.25);
                eve::geometry::axes ax;
            }
        };
